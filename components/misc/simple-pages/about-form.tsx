...
import { OwnerNotFound } from '@/components/info-blocks/owner-not-found'
...
import { GetHostQuery, UserResponse } from '@/graphql/schema/graphql'
...
interface Props {
	hostInfo: NonNullable<GetHostQuery['host']>
	hasOwner: boolean
	className?: string
}
...
export const AboutForm: React.FC<Props> = ({
	...
	const truncatedName = truncateText(
		(hostInfo.owner as UserResponse).name || '',
		20
	)
	const truncatedDescription = truncateText(
		(hostInfo.owner as UserResponse).description || '',
		24
	)
		...
		<div className={cn('', className)}>
			...
			<div className='h-full w-full lg:flex bg-secondary rounded-xl p-4 mt-2'>
				<div className='w-full lg:w-1/2'>
					...
					<Link href={'/u/' + hostInfo.owner.slug}>
						<div className='flex gap-2 mt-2 items-center'>
							<ProfileAvatar
								className='w-14 h-14 border-none bg-secondary hover:bg-secondary'
								avatarImage={
									hostInfo.owner.avatar?.url || '/assets/host/logo.png'
								}
