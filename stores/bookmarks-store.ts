...
interface BookmarksStoreState {
	bookmarksCount: Record<string, number> // Объект для хранения количества лайков по postId
	isAdded: Record<string, boolean> // Объект для хранения состояния лайка по postId
			...
			try {
				if (isAdded[postId]) {
					const response = await fetch(
						`/api/posts/${postId}/bookmarks/delete`,
						{ method: 'POST' }
					)
					...
					set(state => ({
						...
						bookmarksCount: {
							...
							[postId]: (state.bookmarksCount[postId] || 0) - 1,
						},
					}))
				} else {
					const response = await fetch(`/api/posts/${postId}/bookmarks/post`, {
						method: 'POST',
					})
					if (response.status === 401) {
						toast.error('Необходимо авторизоваться', { icon: '❌' })
