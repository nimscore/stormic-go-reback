...
interface CommunityFollowStoreState {
	...
	toggleFollow: (communityId: string) => Promise<void>
	initialize: (communityId: string) => Promise<void>
}

let debounceTimers: Record<string, NodeJS.Timeout | null> = {}
...
export const useCommunityFollowStore = create<CommunityFollowStoreState>(
	(set, get) => ({
		...
		async toggleFollow(communityId: string) {
